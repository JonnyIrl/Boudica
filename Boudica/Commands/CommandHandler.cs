        ...
        public async Task ReactionAddedAsync(Cacheable<IUserMessage, ulong> message, Cacheable<IMessageChannel, ulong> channel, SocketReaction reaction)
        {
            ...
            if (reaction.Emote.Name == "🇯")
            {
                ...
                if(user.Roles.FirstOrDefault(x => x.Name == LightbearerRole) == null && _lightbearerRole != null)
                {
                    ...
                    {
                        await downloadedChannel.SendMessageAsync($"<@{reaction.UserId}>,", embed: EmbedHelper.CreateFailedReply($"you must have <@&{_lightbearerRole.Id}> to join an activity. You can react with this role in <#{WelcomeChannelId}> to get it.").Build());
