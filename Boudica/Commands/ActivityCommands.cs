...
namespace Boudica.Commands
{
    ...
    public class CloseActivityCommands : ActivityHelper
    {
        ...
        public async Task<Result> CloseRaidButtonClick(SocketMessageComponent component, int raidId, ClosedActivityType activityType)
        {
            ...
            {
                await component.RespondAsync(embed: EmbedHelper.CreateSuccessReply($"Raid {embed.Title} has been closed! {updateMessageText}").Build());
            }
            else
            {
                await component.RespondAsync(embed: EmbedHelper.CreateSuccessReply($"Raid Id {raidId} has been closed! {updateMessageText}").Build());
            }
